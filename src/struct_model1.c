...
static void Start_Model_2(s_SM1_i *t_i, s_SM1_f *t_f) {
  uint i;
...
INLINE static void Update_Model_2(uint symbol, s_SM1_i *t_i, s_SM1_f *t_f) {
  uint k, fs, cum;
...
/* Update of the model (twice) for the specified symbol (structured model level
 * 1) */
INLINE static void Update_Model_1(const uint smbl) {
  ...
  k = symbol - 1;
  fs = SM1_f[symbol].freq;
  while ((k > 0) && (fs == SM1_f[k].freq))
    k--;
...
uint32 Zip_SM1(uint32 len, uint8 *bufin) {
  uint32 i = 0;
...
void Unzip_SM1(uint32 len, uint8 *bufout) {
