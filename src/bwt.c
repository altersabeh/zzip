...
INLINE static void ShellSort_depth_2(uint16 **lowerb, uint16 **upperb) {
  uint16 **i;
...
INLINE static void ShellSort(uint16 **lowerb, uint16 **upperb, uint32 depth) {
  uint16 **i;
...
INLINE static void TernarySort(uint16 **lowerb_t, uint16 **upperb_t,
                               uint32 depth_t) {
  sint32 sp = 1;
  uint32 v, depth;
...
void BWT_Coding(uint32 len, uint32 *first, uint8 *buffer8) {
  /* some aliases */
  union {
    uint16 **pointer;
    uint8 *bufinout;
  ...
  p.bufinout = buffer8;
  b.buffer =
      (uint8 *)ROUND32(buffer8 + (len + RUN_LENGTH_MAX) * sizeof(uint16 *));
    ...
    for (j = 0; j < 65535; ++j) {
      if (l > 1) {
        if (l <= (MAX_INSERTSORT + 2))
          ShellSort_depth_2(deb, deb + l - 1);
        else
          TernarySort(deb, deb + l - 1, 2);
      }
      deb += l;
