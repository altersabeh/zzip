...
#ifdef SFX
  printf("*** ZZIP " VERSION_STRING ", SelF-eXtracting archive\n"
         "*** Copyright (c)2001 Damien Debin, all rights reserved.\n");
#else  /* SFX */
  printf("*** ZZIP " VERSION_STRING ", yet another block-sorting compressor.\n"
         "*** Copyright (c)2001 Damien Debin, all rights reserved.\n");
#endif /* SFX */
}
...
INLINE static void MyFree(void *p) { free(p); }

INLINE static void *MyMalloc(uint32 size) {
  void *m = malloc(size);
  if (m == NULL)
...
static uint GetCPUIDflags() {
  uint return_value = 0;

  asm("pushfl;"                     /* get extended flags				*/
      "popl	%%eax;"             /* in eax
                                     */
...
static void GetCpuSpeed() {
  /* POSIX uses struct timeb and ftime(); Windows (MSVC) used _timeb/_ftime */
  struct timeb timeStart, timeStop;
  uint64 StartTicks, EndTicks, TotalTicks;

  /* Wait until near the end of a second to get a consistent start */
  do
    ftime(&timeStart);
...
static void Options(char *s) {
  while (*(++s) != '\0')
    ...
    case '9': {
      ...
      if (*s == 'm') {
        *s = 0;
        block_size = atoi(ss) * 1024 * 1024;
      } else if (*s == 'k') {
        *s = 0;
        block_size = atoi(ss) * 1024;
      } else
        Help();
    } break;
...
static void CmdLineParameters(int argc, char **argv) {
...
static void CheckFilename(char *result, char *filename) {
...
static void DoFiles() {
  ...
  *output_filename = '\0';
  ...
  if (action == NONE) {
    ...
    if (l >= 0 && strcmp(file_list[0] + l, ".zz") == 0)
      action = EXTRACT;
  }

  if ((nb_input_file <= 1) && ((action == DELETE) || (action == CREATE)))
    Help();
#endif /* !SFX */
  ...
  case NONE:
    ...
    for (i = 0; i < nb_input_file; ++i) {
      ...
      path = (char *)MyMalloc(sizeof(char) *
                              (strlen(file_list[i]) + FILENAME_LENGTH_MAX));
      strcpy(path, file_list[i]);

      if (strchr(file_list[i], '*') != NULL)
        go_through = true;
      else {
        if (stat(file_list[i], &file_stat) == -1)
          IO_ERROR();

        if (recurse == true &&
            (file_stat.st_mode & (S_IFDIR | S_IREAD)) == (S_IFDIR | S_IREAD)) {
      ...
      if (go_through == true) {
...
#ifdef WIN32
        {
          ...
          if ((handle = _findfirst(path, &file_info)) == -1)
            IO_ERROR();
          do {
    ...
    for (j = 0; j < nb_archive; ++j) {
      ...
      } else { /* we add to an existing archive */
      ...
      for (i = 0; i < nb_input_file; ++i) {
        ...
        VERBOSE printf("\n  %0*u/%u %s ", l, i + 1, nb_input_file,
                       input_filename);

        AddFile(input_filename, compression_mode, multimedia_test, block_size,
                info);
        if (last_error != OK)
          return;
        ...
        VERBOSE {
          if (info->input_size != 0)
            printf(
                "%+#05.1f%%",
                MIN(((float)(info->output_size) * 100) / info->input_size - 100,
                    0));
          else
            printf("N/A");
        }
  ...
  case EXTRACT: {
    ...
    VERBOSE {
      ...
      if (info->input_size != 0)
        printf(" %+#05.1f%%",
               ((float)(info->output_size) * 100) / info->input_size - 100);
      else
        printf(" N/A");
      printf(" (%u->%u)\n", info->input_size, info->output_size);
    }
  ...
  case LIST:
    ...
    input_filename = file_list[0];
      ...
      namemax = 0;
      for (j = 0; j < nbfile; ++j)
        if (namemax < (sint)strlen(all_files_info[j]->filename))
          namemax = strlen(all_files_info[j]->filename);
      ...
      printf("\n%-*s       date & time   original compressed  ratio"
             "\n%.*s ----------------- ---------- ---------- ------",
             namemax, "filename", namemax, filename);
      ...
      for (j = 0; j < nbfile; ++j) {
        ...
        VERBOSE printf(
            "\n%-*s %s %10d %10d %+#05.1f%%", namemax, info->filename, timestr,
            info->input_size, info->output_size,
            MIN(((float)(info->output_size) * 100) / info->input_size - 100,
                0));
      }
  ...
  case TEST:
    ...
    input_filename = file_list[0];
      ...
      for (j = 0; j < info->nb_of_file; ++j) {
        VERBOSE printf("\n  %0*u/%u %s", l, j + 1, info->nb_of_file,
                       all_files_info[j]->filename);
  ...
  case DELETE: {
    ...
    for (i = 0; i < nbfile; ++i) {
      ...
      if (last_error == FILE_NOT_FOUND) {
        last_error = OK;
        VERBOSE printf("\n  %0*u/%u %s not found", l, i + 1, nbfile,
                       file_list[i]);
      } else {
        if (last_error != OK)
          return;

        VERBOSE printf("\n  %0*u/%u deleting %s", l, i + 1, nbfile,
                       file_list[i]);
...
#define FORMAT1(a)                                                             \
  (float)(a) / (time_stat.cpuspeed >> 16), (float)(a) * 100 / time_stat.time_tot
#define FORMAT2(a, b) (float)(a) * 100 / (b)
#define FORMAT3(a) (float)(a) / (time_stat.cpuspeed >> 16)
#define FORMAT4(a, b)                                                          \
  (float)(a) * ((float)(time_stat.cpuspeed >> 16) / (1024)) / (b)

static void Print_Stat() {
  uint32 bwt = time_stat.time_bwt1 + time_stat.time_bwt2 + time_stat.time_bwt3 +
               time_stat.time_bwt4;
  uint32 st = time_stat.time_st0 + time_stat.time_st1;
  uint32 misc = time_stat.time_tot - bwt - st - time_stat.time_mtf -
                time_stat.time_rle - time_stat.time_txt - time_stat.time_mm -
                time_stat.time_ana;
  ...
  if (time_stat.time_ana != 0)
    printf("\nAnalyzing         = %#7.3fs (%#4.1f%%) %#8.1f kb/s",
           FORMAT1(time_stat.time_ana),
           FORMAT4(time_stat.kb_ana, time_stat.time_ana));
  if (time_stat.time_rle != 0)
    printf("\nRLE (de)coding    = %#7.3fs (%#4.1f%%) %#8.1f kb/s",
           FORMAT1(time_stat.time_rle),
           FORMAT4(time_stat.kb_rle, time_stat.time_rle));
  if (time_stat.time_mm != 0)
    printf("\nMmedia (de)coding = %#7.3fs (%#4.1f%%) %#8.1f kb/s",
           FORMAT1(time_stat.time_mm),
           FORMAT4(time_stat.kb_mm, time_stat.time_mm));
  if (time_stat.time_txt != 0)
    printf("\nTxt/Bin filtering = %#7.3fs (%#4.1f%%) %#8.1f kb/s",
           FORMAT1(time_stat.time_txt),
           FORMAT4(time_stat.kb_txt, time_stat.time_txt));
  if (bwt != 0) {
    printf("\n(un)BWT           = %#7.3fs (%#4.1f%%) %#8.1f kb/s", FORMAT1(bwt),
           FORMAT4(time_stat.kb_bwt, bwt));
    VERBOSE if (time_stat.time_bwt4 != 0) printf(
        " (%.1f%%, %.1f%%, %.1f%%, %.1f%%)", FORMAT2(time_stat.time_bwt1, bwt),
        FORMAT2(time_stat.time_bwt2, bwt), FORMAT2(time_stat.time_bwt3, bwt),
        FORMAT2(time_stat.time_bwt4, bwt));
  }
  if (time_stat.time_mtf != 0)
    printf("\nMTF (de)coding    = %#7.3fs (%#4.1f%%) %#8.1f kb/s",
           FORMAT1(time_stat.time_mtf),
           FORMAT4(time_stat.kb_mtf, time_stat.time_mtf));
  if (st != 0) {
    printf("\nArith. (de)coding = %#7.3fs (%#4.1f%%) %#8.1f kb/s", FORMAT1(st),
           FORMAT4(time_stat.kb_st0 + time_stat.kb_st1, st));
    VERBOSE printf(" (%.1f%%, %.1f%%)", FORMAT2(time_stat.time_st0, st),
                   FORMAT2(time_stat.time_st1, st));
  }
  if (misc != 0)
    printf("\nMisc (i/o,crc...) = %#7.3fs (%#4.1f%%) %#8.1f kb/s",
           FORMAT1(misc), FORMAT4(time_stat.kb_tot, misc));
  printf("\n\nTotal             = %#7.3fs         %#8.1f kb/s\n",
         FORMAT3(time_stat.time_tot),
         FORMAT4(time_stat.kb_tot, time_stat.time_tot));
}
...
int main(int argc, char **argv) {
  ...
  if (last_error != OK) {
    ...
    case FSEEK_OUTPUT_FILE:
      fprintf(stderr, "fseek failed on output file");
      break;
    default:
