...
INLINE static void MyFree(void* p)
{
    free(p);
}
...
static void CmdLineParameters(int argc,
    char** argv)
{
...
static void CheckFilename(char* result,
    char* filename)
{
            ...
            for (i = 0; i < nb_input_file; ++i) {
                ...
                VERBOSE
                {
                    if (info->input_size != 0)
                        printf("%+#05.1f%%", MIN(((float)(info->output_size) * 100) / info->input_size - 100, 0));
                    else
                        printf("N/A");
                }
            }
